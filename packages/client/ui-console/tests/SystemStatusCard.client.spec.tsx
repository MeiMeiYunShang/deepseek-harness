...
describe('SystemStatusCard', () => {
  it('renders each gauge with a live percentage', () => {
    render(<SystemStatusCard t={t} status={{ cpu: 42, memory: 61, gpu: 12 }} />)
  ...
  it('renders N/A when no sample or a null gpu adapter', () => {
    render(<SystemStatusCard t={t} status={null} />)
    expect(screen.getAllByText(en.na, { exact: true }).length).toBe(3)

    cleanup()
    render(<SystemStatusCard t={t} status={{ cpu: 10, memory: 20, gpu: null }} />)
