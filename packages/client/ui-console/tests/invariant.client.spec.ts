...
import * as ConsoleInvariant from '@deepseek-ai/dsh-client-ui-console/invariant'

describe('ui-console invariant companion', () => {
  it('reserves package ownership with an empty installer', async () => {
    const ctx = new Context()
    await ctx.plugin(InvariantRegistry, { enabled: true })

    await expect(ctx.plugin(ConsoleInvariant).await()).resolves.toBeDefined()
  })
