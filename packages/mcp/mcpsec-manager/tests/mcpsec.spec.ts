...
describe('apply wiring', () => {
  ...
  function setup(
    ...
    apply(ctx)
    const handler = rpcHandle.mock.calls[0]?.[1] as ((endpoint: string, payload: unknown) => Promise<unknown>) | undefined
  ...
  it('denies a write-capable MCP tool on a read-only scope', async () => {
    ...
    const decision = await s.preExecute!(exec, async () => ({ kind: 'allow' }))
  ...
  it('adds a stdio server with env/args/cwd/timeout/failOnStartup and normalizes rules', async () => {
    ...
    expect((payload.config as Record<string, unknown>).mcpSecurity).toMatchObject({ toolRules: { a: 'allow', c: 'deny' } })
  })
  ...
  it('ignores a non-MCP tool result and an error with no message', async () => {
    const s = setup()
    s.toolResult!(mkExec('bash', {}), { isError: false } as ToolExecutionResult)
    s.toolResult!(mkExec('mcp__gh__get_issue', {}), errResult('401 unauthorized'))
    s.toolResult!(mkExec('mcp__gh__get_issue', {}), { isError: true } as ToolExecutionResult)
    const stats = await s.handler!('stats', {})
  ...
  it('scores cred/auth/fail/spike/payload/block across servers', async () => {
    ...
    for (let i = 0; i < 3; i++) await s.preExecute!(mkExec('mcp__g2__read'), async () => ({ kind: 'allow' } as PreToolDecision))
  ...
  it('computes a zero rate and no flags for a fully-successful server', async () => {
    const s = setup()
    s.toolResult!(mkExec('mcp__gh__get_issue', {}), { isError: false, value: {} } as ToolExecutionResult)
    const stats = await s.handler!('stats', {})
  ...
  it('handles empty/absent args across handlers', async () => {
    const { handler } = setup()
    expect(await handler!('add', null as unknown as object)).toMatchObject({ ok: false })
  ...
  it('trims the event window past MAX_EVENTS', async () => {
    const s = setup()
    for (let i = 0; i < 2001; i++) s.toolResult!(mkExec('mcp__gh__get_issue'), { isError: false, value: {} } as ToolExecutionResult)
    const stats = await s.handler!('stats', {})
    expect(stats).toMatchObject({ ok: true, value: { servers: expect.any(Array) } })
  })

  it('measures a duration across a pre-execute/result pair', async () => {
    const s = setup()
    const exec = mkExec('mcp__gh__get_issue')
    await s.preExecute!(exec, async () => ({ kind: 'allow' } as PreToolDecision))
    s.toolResult!(exec, { isError: false, value: {} } as ToolExecutionResult)
    const stats = await s.handler!('stats', {})
  ...
  it('handles null args across npmSearch/setScope/setEnabled/edit', async () => {
    ...
    expect(await s.handler!('npmSearch', null as unknown as object)).toMatchObject({ ok: true })
    vi.unstubAllGlobals()
    expect(await s.handler!('setScope', null as unknown as object)).toMatchObject({ ok: false })
    expect(await s.handler!('setEnabled', null as unknown as object)).toMatchObject({ ok: false })
    expect(await s.handler!('edit', null as unknown as object)).toMatchObject({ ok: false })
  })
  ...
  it('setScope normalizes a non-object config and merges over existing rules', async () => {
    ...
    const s = setup([
      ...
      entry({}, { options: { id: 's1', name: MCP, config: 'non-object' as unknown as Record<string, unknown> } }),
    ])
