...
const NO_MODEL_EXPERIENCE_SECTION: Readonly<Record<string, string>> = {
  ...
  'packages/client/ui-console': 'The console is a browser-side monitoring surface that forwards Smart Q&A requests verbatim through the remote LLM service and registers nothing model-facing of its own.',
}
...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/host/host-metrics': { kind: 'none', reason: 'The sampler emits a host telemetry event consumed only by the browser console system-status panel; the metric never enters the model context or session log.' },
