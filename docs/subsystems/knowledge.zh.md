...
由 `packages/knowledge/*` 包拥有的持久化用户笔记与分组。条目包含可搜索文本、标签与可选分组；分组用于组织条目，并在写入时指定。文件后端负责持久化；控制器通过远程网关向浏览器设置 UI 暴露服务。

来源：[`packages/knowledge/knowledge/src/index.ts`](../../packages/knowledge/knowledge/src/index.ts)、[`packages/api/knowledge-controller/src/index.ts`](../../packages/api/knowledge-controller/src/index.ts)、[`packages/knowledge/knowledge-file/src/index.ts`](../../packages/knowledge/knowledge-file/src/index.ts)、[`packages/knowledge/tool-knowledge/src/index.ts`](../../packages/knowledge/tool-knowledge/src/index.ts)

## 持久化记录

`KnowledgeEntry` 是持久化笔记：包含 `id`、`title`、`content`、`category`、可选 `groupId`、`tags` 与时间戳。`KnowledgeGroup` 按 `name` 聚合条目。服务封装创建、更新、删除、列表与分组分配；文件后端将每个条目写入独立 JSON 文件并维护分组索引，随后发出 `knowledge/change` 事件以供缓存失效。

## 工具面

`dsh-tool-knowledge` 暴露两个模型可调用的工具：`knowledge/search` 返回按文本相关度排序的匹配条目，`knowledge/save` 记录新条目。工具读取知识服务，将结果格式化为上下文，并在请求模型总结大量内容以持久化前追加 `knowledge/summary-llm-request` 日志事件。
